...
int main (int argc, char *argv[])
{
  ...
  csmaNodes.Add (p2pNodes.Get (2));  // c  at node n2
  ...
  p2pDevices1 = pointToPoint.Install (p2pNodes.Get(0),p2pNodes.Get(1)); //c
  p2pDevices2 = pointToPoint.Install (p2pNodes.Get(1),p2pNodes.Get(2)); //c
  ...
  p2pInterfaces2 = address.Assign (p2pDevices2);  //i
  ...
  csma.EnableAsciiAll (ascii.CreateFileStream ("l2q1.tr"));  //i

  // pointToPoint.EnablePcapAll ("second");
  // csma.EnablePcap ("second", csmaDevices.Get (1), true);
  AnimationInterface anim ("l2q1.xml");   //i
  Simulator::Run ();
  Simulator::Destroy ();
  return 0;
}
